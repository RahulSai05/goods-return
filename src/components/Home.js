    ...
    try {
      ...
      if (response.status === 200) {
        if (response.data.overall_condition) {
          ...
          setStep(2);
          setMessage(
            "Front image has been saved. Now upload the back view image of the product."
          );
        }
      } else {
    ...
    <div style={styles.container}>
      ...
      <div style={styles.mainContent}>
        ...
        <p style={styles.description}>
          Auditly offers an AI-driven solution to simplify and enhance your
          product return process. Experience efficiency and transparency
          like never before.
        </p>
        {!result && <p style={{color:'white'}}>{message}</p>}
        {step === 0 && !result && (
